...
Anchor 0 5V           -> 独立稳定5V电源
```

固件默认禁用UART TX，只接收Anchor 0测距数据。GPIO18已经按照片中的ESP32-S3-N16R8开发板分配为UWB RX。

数字钥匙是唯一移动Tag；门锁内只配置Anchor 0和Anchor 1。ESP32-S3只接Anchor 0的UART2，Anchor 1只需可靠供电，两块Anchor的TX不能并联。
...
请保存连续10~20帧日志，并同时记录真实距离、Tag/Anchor ID、固件版本和配置。解析器按官方V1.0.1协议处理37字节帧；首次实物日志用于复核厂家文档中表述不够明确的校验覆盖范围。
...
test_contest_scenario.c会生成左右480 mm基线下的双Anchor理想测距，完整模拟钥匙从3 m进入1.75 m迎宾区、0.8 m开锁区，再跨越1 m和2 m离开。测试同时覆盖前后镜像选择、不可相交测距、1 m附近抖动滞回、50度角度拒绝、ID失配和信号丢失闭锁。

流水线入口定义在`c_key_pipeline.h`。串口解析层需要填入Anchor 0和Anchor 1的距离、时间戳、流水号和Tag ID；输出直接给出滤波距离、二维位置、径向距离、方位角、门锁状态和动作事件。

## UART2到门锁状态的完整链路

主程序已经通过c_key_bu03_bridge把UART2的A0/A1两路毫米距离自动送入定位流水线。运行时每秒输出一次与TFT一致的6行状态；进入或离开区域、开锁或闭锁以及UWB恢复或超时会立即输出。
...
可配置两块Anchor的X/Y坐标、两路距离零偏、圆柱半径、正前方角度修正、EMA系数和最大定位残差。默认A0=(-240,0) mm、A1=(240,0) mm，门锁正前方为+Y；圆柱半径为300 mm。
...
当前优先事项是备份三块BU03配置、确认蜂鸣器硬件驱动、完成正式供电、改善左右角度不对称，并在最终机械安装后重新标定。完整状态和验收清单见：
