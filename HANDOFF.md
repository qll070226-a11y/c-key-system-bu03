...
- 遥测升级为 C_KEY_DIAG_V3，并保持对V2回放兼容。
- 上位机升级为 PDOA 距离/角度诊断，并增加静态定位正式验收工具，18项测试通过。
- 2026-08-02复测C核心测试和Python上位机测试均通过。
- ESP-IDF 5.5.4完整构建通过，完赛2版镜像为0x44280字节。
- 完赛2版固定在提交8f133a0、标签final-v2-20260801；负角度校准前的
  完赛1版固定在提交465fcba、标签final-v1-20260801。
...
- 正式12V电池、两路DCDC、最终洞洞板和装箱未形成验收记录。
- 负角度分段校准已经写入完赛2版，但尚缺1/2/3m距离及0、正负30、正负45度
  的统一正式报告。
- 动态双向边界、实物拨码ID失配/恢复、重启和30分钟老化尚缺正式报告。
...
正前方为0度、+Y；右侧为正角和+X。当前距离scale=1、offset=0、安装角度零偏=0。
完赛2版另使用负角度分段校准节点；未经重新完整采样不得修改这些固定参数。
...
负角度实测标定节点为原始-12.3/-28.2/-41.5度对应物理-15/-30/-45度；
采用单调分段线性插值，0度及正角度保持恒等映射。
...
6. 按[比赛验收状态](docs/CONTEST_ACCEPTANCE_STATUS.md)完成剩余实机报告。
